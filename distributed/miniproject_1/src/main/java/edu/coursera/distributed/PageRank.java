...
public final class PageRank {
    ...
    public static JavaPairRDD<Integer, Double> sparkPageRank(
        ...
        final JavaPairRDD<Integer, Double> newRanks = sites.join(ranks)
                .flatMapToPair(pair -> {
                    Website website = pair._2._1;
                    Double rank = pair._2._2;
                    List<Tuple2<Integer, Double>> nextWebsites = new ArrayList<>();
                    Iterator<Integer> edgeIterator = website.edgeIterator();
                    final int number = website.getNEdges();
                    while (edgeIterator.hasNext()) {
                        Integer nextWebsite = edgeIterator.next();
                        nextWebsites.add(new Tuple2<>(nextWebsite, rank / number));
                    }
                    return nextWebsites;
                })
