...
public final class ParBoruvka extends AbstractBoruvka<ParBoruvka.ParComponent> {
    ...
    public void computeBoruvka(final Queue<ParComponent> nodesLoaded,
        ...
        ParComponent c1;
        while ((c1 = nodesLoaded.poll()) != null) {
            if (c1.isDead || !c1.lock.tryLock()) {
                continue;
            }
            ...
            Edge<ParComponent> e = c1.getMinEdge();
            if (e == null) {
                solution.setSolution(c1);
                c1.lock.unlock();
                break;
            }

            ParComponent c2 = e.getOther(c1);

            if (c2.isDead || !c2.lock.tryLock()) {
                c1.lock.unlock();
                nodesLoaded.add(c1);
                continue;
            }
